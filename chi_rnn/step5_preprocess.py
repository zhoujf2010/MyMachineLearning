...
# from music21 import *
from music21 import converter,stream,instrument,tempo,key,meter,note,chord,interval,scale
...
def __parse_midi(data_fn):
    ...
    for _, group in groupby(offsetTuples, lambda x: x[0]):
    ...
    chords = OrderedDict()
    measureNum = 0
    for _, group in groupby(offsetTuples_chords, lambda x: x[0]):
...
def unparse_grammar(m1_grammar, m1_chords):
    ...
    prevElement = None
    for _, grammarElement in enumerate(m1_grammar.split(' ')):
