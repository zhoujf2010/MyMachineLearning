...
if __name__ == '__main__':
    ...
    n = 300
    x = np.random.randn(n, 50)  #产生300个50个维度的数据
    y = np.array([0]*100+[1]*100+[2]*100)   #前100个为0，中间100个为1，后100个为2， 三分类
    n_class = 3 
#     print x.shape
#     print x
    print 'before y=',y
    ...
    clf.fit(x, y)
    y_score = clf.decision_function(x)  #做了3次logistic回归来实现soft-max回归
    ...
    # micro
    fpr['micro'], tpr['micro'], thresholds = metrics.roc_curve(y.ravel(), y_score.ravel())  #y拉成直线 一条数据（整个当成二分类处理）
