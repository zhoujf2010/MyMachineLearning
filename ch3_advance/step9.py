...
if __name__ == '__main__':
    ...
    mode1 = RidgeCV(alphas=np.logspace(-3, 2, 50))
    mode1.fit(x2,y)
    print '超参数:', mode1.alpha_
    x_hat = np.linspace(min(x), max(x), 100)
    x_hat.shape = -1, 1
    x_hat2 =  pmode.fit_transform(x_hat)
    y_hat = mode1.predict(x_hat2)
    ...
    #Lasso
    mode1 = LassoCV(alphas=np.logspace(-3, 2, 50))
    mode1.fit(x2,y)
    print '超参数:', mode1.alpha_
    x_hat = np.linspace(min(x), max(x), 100)
    x_hat.shape = -1, 1
    ...
    #ElasticNet
    mode1 = ElasticNetCV(alphas=np.logspace(-3, 2, 50), l1_ratio=[.1, .5, .7, .9, .95, .99, 1])
    mode1.fit(x2,y)
    print '超参数:', mode1.l1_ratio_,mode1.alpha_
    x_hat = np.linspace(min(x), max(x), 100)
    x_hat.shape = -1, 1
    x_hat2 =  pmode.fit_transform(x_hat)
    y_hat = mode1.predict(x_hat2)
